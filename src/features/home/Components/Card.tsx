...
export const Card = (destination: IDestinationProps) => {
  const { id, name, country, city, logo, cover, currentLocation, location } =
    destination;
        ...
        <div className="relative overflow-hidden">
          <Carousel>
            {carouselImages.map((image, index) => (
              <Link key={index} to={`/destinations/${id}`}>
                <img
                  key={index}
      ...
      <div className="flex flex-col">
        <div className="flex items-center justify-between">
          <Tooltip title={destinationName} placement="top">
            <h1 className="w-[85%] overflow-hidden text-[15px] font-bold whitespace-nowrap text-ellipsis font-onest">
              {destinationName}
            </h1>
          </Tooltip>
        ...
        <div className="flex flex-col font-onest">
          ...
          <span className="mt-1 text-sm text-[#6A6A6A]">
            <span className="font-semibold text-[#000]">$500 USD</span> per
            night
          </span>
        </div>
      </div>
